...
function! bitly#http#decodeURI(str)
  let ret = a:str
...
function! bitly#http#encodeURIComponent(items)
  let ret = ''
  if type(a:items) == 4
    for key in sort(keys(a:items))
      if strlen(ret) | let ret .= "&" | endif
      let ret .= key . "=" . bitly#http#encodeURIComponent(a:items[key])
    endfor
  elseif type(a:items) == 3
    for item in sort(a:items)
      if strlen(ret) | let ret .= "&" | endif
...
function! bitly#http#get(url, ...)
  ...
  let option = a:0 > 2 ? a:000[2] : {}
  let url = a:url
  let getdatastr = bitly#http#encodeURI(getdata)
  ...
  for opt_key in keys(option)
    let command .= " " . opt_key . " " . option[opt_key]
  endfor
...
function! bitly#http#post(url, ...)
  let postdata = a:0 > 0 ? a:000[0] : {}
  let headdata = a:0 > 1 ? a:000[1] : {}
  let method = a:0 > 2 ? a:000[2] : "POST"
  let option = a:0 > 3 ? a:000[3] : {}
  let url = a:url
  if type(postdata) == 4
    let postdatastr = bitly#http#encodeURI(postdata)
  else
  ...
  let res = system(command . " -c /tmp/cookie --data-binary @" . quote.file.quote)
  call delete(file)
  if res =~ '^HTTP/1.\d 3' || res =~ '^HTTP/1\.\d 200 Connection established'
    let pos = stridx(res, "\r\n\r\n")
    if pos != -1
